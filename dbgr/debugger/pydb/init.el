...
(defvar dbgr-pydb-pat-hash (make-hash-table :test 'equal)
...
(setf (gethash "prompt" dbgr-pydb-pat-hash)
      (make-dbgr-loc-pat
       :regexp   "^[(]+Pydb[)]+ "
       ))

;;  Regular expression that describes a Python backtrace line.
(setf (gethash "lang-backtrace" dbgr-pydb-pat-hash) 
...
(setf (gethash "brkpt-set" dbgr-pydb-pat-hash)
      (make-dbgr-loc-pat
...
(setf (gethash "brkpt-del" dbgr-pydb-pat-hash)
      (make-dbgr-loc-pat
       :regexp "^Deleted breakpoint \\([0-9]+\\)\n"
       :num 1))

(setf (gethash "font-lock-keywords" dbgr-pydb-pat-hash)
      '(
...
(setf (gethash "pydb" dbgr-pat-hash) dbgr-pydb-pat-hash)

(defvar dbgr-pydb-command-hash (make-hash-table :test 'equal)
  "Hash key is command name like 'shell' and the value is 
  the pydb command to use, like 'python'")

(setf (gethash "shell" dbgr-pydb-command-hash) "python")
(setf (gethash "pydb" dbgr-command-hash) dbgr-pydb-command-hash)
